...
- **Envenenamiento de historial por neutralización (bug 2026-07-30):** el neutralizador post-LLM reemplazaba el output y el nodo guardaba ESE contenido (`result.content`) como turno assistant en `conversation_history`. El LLM, al ver TODOS sus turns anteriores con la frase "Entendido, dime cómo puedo ayudarte...", la imita — y la frase imitada ya no matchea la regex del neutralizador, perpetuando el bucle. Fix: guardar `result.rawOriginal` (output real del LLM pre-neutralización) en el historial. Regla de oro: lo que se guarda en el historial debe ser lo que el LLM REALMENTE dijo, nunca el rewrite post-proceso.
- **Síntoma de historial envenenado:** si el bot repite la MISMA frase exacta en cada turno a pesar de fixes en el código, el problema casi nunca es el código actual — es el `conversation_history` en DB lleno de la frase vieja. Limpiar con `DELETE FROM conversation_history WHERE numero='<número>'` (también revisar otros números con `messages::text LIKE '%frase%'`).
- **Acceso a Postgres del VPS:** el `PG_PASSWORD` del `.env` local NO es la del servidor. La real está en `/root/.env` del VPS o en el env del contenedor (`docker exec meyer_postgres env | grep POSTGRES_PASSWORD`). Requiere túnel SSH: `ssh -f -N -L 5432:localhost:5432 root@178.104.27.180`.
