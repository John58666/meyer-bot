...
> Última actualización: 28 de junio de 2026 (post Sprint 6 — fixes dashboard + presentación Brayan).
...
│   │   └── new-appointment-sheet.tsx    # Formulario crear cita: servicios dinámicos con precio, hora libre
│   ├── lib/
│   │   ├── db.ts
...
- Todos los días reales del calendario son clickeables. Sheet decide según fecha: con citas + hoy/futuro → lista + CTA agendar; vacío + hoy/futuro → empty state + CTA; pasado → sin CTA.
- `NewAppointmentSheet` refactorizado: servicios dinámicos con precio desde `services_text`, fecha precargada desde calendario, hora libre `<input type="time">`, anti-doble-booking con warning suave + override.
- `services_text` se fetcha desde DB en cada page server component (paralelo con otras queries). No está en JWT.
- `revalidatePath("/dashboard/semana")` ES CORRECTO — NO cambiar a `/semana`. La URL real es `/dashboard/semana` porque `(dashboard)` es route group (invisible en URL) pero `dashboard` es carpeta real.
...
El parser `parseServices` muestra nombre + precio en cada opción del select. El dueño ve "Corte caballero $18.000" al agendar manualmente. Útil también para Fix 5 (métricas de ingresos).
...
#### Correcciones de datos en DB (no son bugs de código)
- `businesses.name` de Meyer estaba corrupto ("Barbería Brayan") → corregido a "Peluquería Meyer"
- `businesses.services_text` de Meyer tenía formato con guiones → corregido a formato coma-separado
- `businesses.services_text` de Brayan tenía formato con guiones → corregido a formato coma-separado
- El título del topbar y los servicios del formulario son automáticos por negocio — no requieren código por cliente
...
- `services_text` formato: `"Nombre $precio, Nombre2 $precio2"` — coma como separador, precio con `$`. Parseado en dashboard con split+regex. No hay tabla `services` aún.
...
- Servicios actuales en DB: `Corte caballero $18.000, Corte+barba $22.000, Barba $10.000, Cejas $5.000`
- ⚠️ **Pendiente confirmar con Brayan:** los nombres reales son "Corte de caballero (cejas incluidas) $18.000, Corte de caballero + barba (cejas incluidas) $22.000, Barba $10.000, Cejas $5.000". Cuando confirme, UPDATE:
  ```sql
  UPDATE businesses
  SET services_text = 'Corte de caballero (cejas incluidas) $18.000, Corte de caballero + barba (cejas incluidas) $22.000, Barba $10.000, Cejas $5.000'
  WHERE id = 3;
  ```
...
- **Problema crítico:** el workflow `Leer Slots Disponibles` en n8n **NO consulta `schedule_exceptions`**. Solo mira `schedule_text` y `appointments`. Implementarlo requiere:
  ...
  2. Modificar el SQL de `Leer Slots Disponibles` para excluir slots con excepción activa.
- Feature que cruza dashboard + n8n. Sprint dedicado. Alta prioridad de venta.

## Feature pendiente — Edición de servicios desde dashboard
El dueño necesita poder editar sus servicios y precios desde el dashboard sin acceso directo a la DB.
- **Problema de sincronización:** `services_text` lo lee el AI Agent en el system prompt del bot. Cualquier cambio en el dashboard debe reflejarse inmediatamente en lo que el bot ofrece.
- El bot ya lee `services_text` desde la DB en cada turno (no está cacheado), así que el UPDATE es suficiente — no requiere cambios en n8n.
- Requiere UI de edición en el dashboard + validación del formato `"Nombre $precio, ..."`.
- Sprint dedicado. Diseñar antes de implementar.
...
- Vista "Hoy" — stats (Total/Pendientes/Completadas/Canceladas) + lista de citas + botón nueva cita
...
- `services_text` se carga desde DB en `semana/page.tsx` y `dashboard/page.tsx` en paralelo con otras queries

## Onboarding de nuevo negocio (proceso manual actual)
1. SQL INSERT en `businesses` con todos los campos incluyendo `schedule_text` JSONB y `services_text` en formato `"Nombre $precio, ..."`
...
6. El título del dashboard, servicios del formulario y UI multi-barbero son automáticos por negocio — no requieren código por cliente.
...
- Dashboard: título dinámico por negocio, calendario clickeable, agendar manual con servicios dinámicos y precios, hora libre, anti-doble-booking
...
2. **Bloqueo de agenda** — UI en dashboard + JOIN en `Leer Slots Disponibles` del workflow n8n. Alta prioridad de venta.
3. **Edición de servicios desde dashboard** — con sync automático al bot (el bot ya lee `services_text` desde DB en cada turno, no requiere cambios en n8n). Requiere diseño de UI + validación de formato.

### 🟡 DESPUÉS
4. **Password fuerte con Bitwarden** — reemplazar el placeholder en los 3 lugares
5. **CRM** — upsert automático en `customers` al agendar por WhatsApp + UI dashboard
6. **Fix 3 dashboard** — verificar en producción que sync cancelación WhatsApp → dashboard funciona con el polling actual
7. **Notificación al dueño con nombre del cliente** — lookup en `customers` en `Construir Mensajes`
8. **Quitar branding "Meyer"** del producto — renombrar instancia `peluqueria-beta`, `whatsapp_instance`, workflow en n8n, referencias en repo
9. **Confirmar `services_text` de Brayan** con el cliente y aplicar UPDATE (ver sección Brayan Study)

### 🟠 ALTA PRIORIDAD
10. **Multi-barbero completo** — sprint dedicado (ver sección)
11. **`Confirmar Reagendamiento` → Raw body** — deuda técnica; hoy usa IIFE en bodyParameters
12. Gestión de no-shows: cron que detecta citas pasadas sin marcar + aviso al barbero
13. `reminder_config` JSONB en businesses: recordatorios configurables por negocio
14. Panel admin para Johnander (ver todos los negocios, métricas agregadas)
15. Google private key fuera del .env del VPS
16. Evolution API expuesta en 0.0.0.0:8080 — asegurar con firewall

### 🟢 MEJORAS FUTURAS
17. Migración a WhatsApp Cloud API oficial (botones interactivos, mayor escala)
18. Prompt caching (reducir el ~33K tokens/conversación a escala)
19. Timezone dinámico por negocio
20. Mover DNS a Cloudflare
21. Rate limit en PostgreSQL (persistente entre reinicios)
22. Facturación con Stripe/Wompi
23. Migración n8n → Node.js + BullMQ + Redis (30+ clientes)
24. Upgrade VPS Hetzner (4 vCPU / 8GB) a los 8-10 clientes activos
25. Tabla `services` normalizada (reemplaza `services_text` string)
26. Expansión regional: España y EEUU
...
- **Claude.ai debe verificar hechos antes de confirmar diagnósticos** — en Sprint 6 claude.ai tuvo un error de diagnóstico en `revalidatePath` que Claude Code siguió sin verificar. Claude Code debe chequear rutas/hechos reales antes de asumir que el diagnóstico es correcto.
...
- **Confirmar variables en runtime:** `docker exec [container] env | grep [VAR]` (más fiable que mirar `.env`).
...
- **numero vs numeroLimpio** — `numero` incluye `@s.whatsapp.net`; `numeroLimpio` solo dígitos.
...
- **Next.js route groups** `(nombre)` NO aparecen en la URL. Solo las carpetas reales cuentan. `app/(dashboard)/dashboard/semana/page.tsx` → URL `/dashboard/semana`. NO confundir al escribir `revalidatePath`.
...
- **Datos corruptos en DB** pueden simular bugs de código — siempre verificar el dato en DB antes de asumir que es un bug de lógica.
